import { BehaviorOption, OttWebsocketError, Role } from "ott-common/models/types";

export default {
	"edge-preview": {
		notice: "Cloudflare 预览版 · 支持视频直链 · 使用独立房间与浏览器身份",
		guest: "访客预览",
		ownership:
			"创建者在当前浏览器中拥有房主权限。清除站点数据或超过 30 天未访问可能失去此身份；预览版暂不支持账号登录和身份找回。",
		"connection-failed": "暂时无法连接，请重试。",
		"add-label": "视频链接",
		"add-placeholder": "粘贴 MP4、M3U8、MPD 或自定义媒体 JSON 链接",
		"intro-text3": "目前，你可以和朋友一起观看 MP4、HLS、DASH 直链或自定义媒体清单的视频，",
		"media-help":
			"支持 MP4、HLS、DASH 和自定义媒体清单。请使用允许本站播放的 HTTPS 直链，视频由原片源直接加载。",
		"try-video": "试播示例视频",
	},
	common: {
		retry: "重试",
		"nav-menu": "导航菜单",
		"skip-content": "跳到内容",
		more: "更多",
		language: "语言",
		yes: "是",
		no: "否",
		ok: "确定",
		cancel: "取消",
		close: "关闭",
		"close-all": "全部关闭",
		"more-notices": "另有 {count} 条新提示",
		add: "添加",
		remove: "移除",
		delete: "删除",
		play: "播放",
		pause: "暂停",
		save: "保存",
		search: "搜索",
		undo: "撤销",
		copy: "复制",
		show: "显示",
		hide: "隐藏",
		discard: "放弃",
		loading: "加载中…",
		view: "查看",
		restore: "恢复",
		success: "成功",
		vote: "投票",
		unvote: "取消投票",
		on: "开",
		off: "关",
		dismiss: "忽略",
		errors: {
			"rate-limit": "请求过于频繁，请在 {duration} 秒后重试。",
		},
	},
	behavior: {
		[BehaviorOption.Always]: "始终",
		[BehaviorOption.Prompt]: "询问",
		[BehaviorOption.Never]: "从不",
	},
	landing: {
		hero: {
			eyebrow: "下一场观影，和朋友一起",
			title: "好电影，一起看。",
			description:
				"创建一个房间，分享一条链接。和朋友同步播放、边看边聊，把相聚留给每一个好故事。",
			btns: {
				create: "@:nav.create.title",
				browse: "浏览房间",
				source: "查看源码",
			},
		},
		intro: {
			title: "远程观影派对，变得轻松多了",
			name: "OpenTogetherTube",
			text1: "是一个实时视频同步平台。\n使用简单，无需注册。只需创建房间、添加视频并\n邀请你的朋友。砰！你就可以和朋友一起刷视频刷到凌晨三点了。",
			text2: "最初的 TogetherTube 因其简洁的界面而广受喜爱，\n还有立刻就能开始看视频的便捷。\nOpenTogetherTube 希望做到同样简单，并在此基础上\n进一步改进，让它变得更好。",
			text3: "目前，你可以和朋友一起观看来自 Youtube、Vimeo、.mp4 视频直链的在线视频，以及",
			link: "更多功能正在路上",
		},
		features: {
			title: "核心功能",
			"synchronized-playback": {
				title: "同步播放",
				text: "你按下播放，视频就会为房间里的\n每个人播放。就这么简单。",
			},
			"permanent-rooms": {
				title: "永久房间",
				text: "保存当前视频、播放进度和待播列表。离开时自动暂停，下次回到同一个房间继续看。",
			},
			"dark-theme": {
				title: "深色主题",
				text: "深夜看视频合集？\nOpenTogetherTube 默认采用深色主题，\n让你的眼睛不再受罪。",
			},
			"room-permissions": {
				title: "房间权限",
				text: "厌倦了陌生人随意进入你的房间，\n往你惬意的 lo-fi 听歌时光里\n加一堆吵闹的视频？直接禁止他们添加视频就好。",
			},
			"voting-system": {
				title: "投票系统",
				text: "决定不了接下来看什么？把队列\n切换到投票模式，让民主发挥\n它最擅长的作用。",
			},
			"playlist-copying": {
				title: "播放列表复制",
				text: "一次性把整个播放列表或频道\n添加到队列，不必坐在那里\n一个一个地添加。这是和朋友\n一起刷新频道最好的方式。",
			},
		},
		support: {
			title: "支持开发",
			description1: "没有像你这样的贡献者和支持者的帮助，OpenTogetherTube 就不可能实现。",
			description2:
				"通过贡献你的想法或代码来参与开发，或成为赞助者来表达支持。所有捐款都用于支付托管费用、OpenTogetherTube 的开发，以及保持 OpenTogetherTube 无广告。",
			how: "我能帮上什么忙？",
			sponsor: "成为赞助者",
			contribute: "贡献代码",
		},
	},
	footer: {
		upstream: "上游项目",
		"download-source": "下载当前版本源码",
		disclaimer: "免责声明：OpenTogetherTube 项目与 TogetherTube 及 Watch2Gether 无关。",
		"made-in": "美国制造",
		"thanks-to": "特别鸣谢",
		"privacy-policy": "隐私政策",
		attribution: "归属声明",
	},
	"not-found": {
		title: "页面未找到",
		home: "@:nav.home",
		browse: "@:landing.hero.btns.browse",
	},
	"quick-room": {
		text: "正在为你创建临时房间…",
	},
	attribution: {
		"sponsorblock-text": "使用来自以下来源的 SponsorBlock 数据",
	},
	"page-title": {
		site: "一起看 · OpenTogetherTube 中文版",
		room: "观影房间",
		"password-reset": "重置密码",
		playground: "组件预览",
		themes: "主题预览",
	},
	nav: {
		home: "首页",
		browse: "浏览",
		"my-rooms": "我的房间",
		account: "账户",
		faq: "常见问题",
		bug: "报告 Bug",
		support: "支持上游开发",
		login: "登录",
		"link-discord": "关联 Discord",
		logout: "登出",
		create: {
			title: "创建房间",
			temp: "创建临时房间",
			"temp-desc": "尽快和朋友一起看视频。",
			perm: "创建永久房间",
			"perm-desc": "空房自动暂停，保存视频和待播列表，再次进入时恢复。",
		},
	},
	"my-rooms": {
		"owned-eyebrow": "你的放映厅",
		"no-rooms": "你还没有房间。",
		"delete-label": "删除房间 {name}",
		"delete-failed": "删除房间失败，请重试。",
		"confirm-delete": "永久删除房间“{name}”？此操作无法撤销。",
	},
	account: {
		title: "账户",
		description: "管理与你账户关联的邮箱和密码。",
		details: "账户详情",
		social: "社交账户",
		discord: "Discord",
		linked: "已关联",
		"not-linked": "未关联",
		"link-discord": "关联 Discord",
		"unlink-discord": "取消关联 Discord",
		"discord-unlinked": "已取消关联 Discord。",
		"discord-unlink-requires-password": "取消关联 Discord 前需要先设置密码。",
		username: "用户名",
		email: "邮箱",
		"no-email": "尚未添加邮箱。",
		"add-email": "添加邮箱",
		"change-email": "修改邮箱",
		"save-email": "保存邮箱",
		"add-password": "添加密码",
		"change-password": "修改密码",
		"current-password": "当前密码",
		"current-password-required": "需要输入当前密码",
		"save-password": "保存密码",
		"email-saved": "邮箱已更新。",
		"password-saved": "密码已更新。",
		"save-failed": "无法保存账户更改。",
		"load-failed": "无法加载账户。",
	},
	"room-list": {
		"browse-eyebrow": "公共放映厅",
		"empty-eyebrow": "等你开场",
		"load-failed": "暂时无法加载房间，请重试。",
		viewers: "{count} 人在线",
		"permanent-room": "永久房间",
		enter: "进入房间",
		"no-rooms": "暂时没有房间…",
		create: "@:nav.create.title",
		"no-description": "暂无描述。",
		"nothing-playing": "没有正在播放的内容。",
	},
	room: {
		"title-temp": "临时房间",
		"kick-me": "踢出我",
		rewind: "后退 10 秒",
		skip: "前进 10 秒",
		"play-pause": "播放/暂停",
		"next-video": "下一个视频",
		"next-video-vote": "投票跳过视频",
		"toggle-fullscreen": "切换全屏",
		"fullscreen-hint": "浏览器未允许自动进入全屏，可点击全屏按钮手动进入。",
		upscale: {
			title: "画质增强",
			off: "关闭",
			sharpen: "清晰化（锐化放大）",
			film: "影视（去噪去带）",
			anime4k: "AI 超分（Anime4K）",
			"anime4k-quality": "AI 超分（质量）",
			"webgpu-fallback": "此设备不支持 WebGPU，已切换为「清晰化」。",
			"film-fallback": "影视档无法运行，已切换为「清晰化」。",
			"anime4k-fallback": "AI 超分无法运行，已切换为「清晰化」。",
			"anime4k-quality-fallback": "「质量」档无法启用，已切换为「AI 超分」。",
			failed: "无法启用画质增强，已关闭。",
			degraded: "设备性能不足，画质增强已降低。",
			help: "画质增强说明",
			"intro-sharpen":
				"清晰化：GPU 边缘自适应放大（EASU）加锐化，所有设备都能用；倍率贴合显示尺寸且不低于原分辨率，最省电。效果偏轻微，低分辨率或发糊的片源更明显。",
			"intro-film":
				"影视：先用保边去噪清掉压缩块与蚊噪、按邻域跨度压平色带并加抖动，再做 EASU 放大与收敛锐化（锐化强度按 0.6 计）。真人剧集与电影建议用它；动画片用下面的 AI 超分。只需 WebGL2，手机也能开。",
			"intro-anime4k":
				"AI 超分（Anime4K）：用 WebGPU 神经网络重建线条，适合动漫；独立显卡余量充足，集显和手机可能吃力。",
			"intro-anime4k-quality":
				"AI 超分（质量）：在快速档之上再叠一遍修复与放大（A+A），感知质量最高，GPU 开销约为快速档两倍；笔记本会明显发热、风扇转速上升，建议在 2× 及以上倍率使用。",
			"intro-note": "性能不足时会自动逐级降档；在下方「进阶」里关闭自动降档可保持当前设置。",
			advanced: "进阶",
			strength: "锐化强度",
			scale: "渲染倍率",
			"scale-auto": "自动（电脑 2× 源，触屏贴合显示）",
			"scale-hint":
				"自动不会低于原分辨率；电脑（鼠标设备）会渲染到 2× 源再缩到屏幕，AI 超分的放大阶段因此才真正生效。大屏上更高倍率更锐利、也更耗电；设备吃力时调低可明显省性能。",
			"auto-degrade": "性能不足时自动降档",
			"auto-degrade-hint": "关闭后将保持当前设置，即使掉帧也不自动降低。",
		},
		pip: "画中画",
		"player-settings": "播放器设置",
		subtitles: "字幕/CC",
		quality: "画质",
		"playback-speed": "播放速度",
		"buffer-gate-waiting": "已暂停，缓冲中：{names}",
		"default-layout": "默认布局",
		"theater-mode": "影院模式",
		"con-status": {
			connecting: "连接中…",
			connected: "已连接",
			timeout: "连接超时，正在重试…",
			reconnecting: "连接中断，正在重试…",
			retry: "立即重连",
			"token-retry": "获取身份凭证失败（网络波动或限流），正在自动重试…",
			"network-help":
				"先分辨线路问题：开了代理的话，请将本站地址加入代理的绕过列表，或改用支持 WebSocket 的代理；没开代理则换个网络（例如手机热点、另一家运营商）再试。本站经 Cloudflare 对外，部分网络到该线路的 WebSocket 会被限速或阻断。网络恢复后会自动重连。",
		},
		"visibility-badge-label": "房间可见性",
		"visibility-unknown": "未知可见性",
		"voice-join": "加入语音",
		"voice-mute": "静音",
		"voice-leave": "退出语音",
		"voice-participants": "{count} 人在语音",
		"voice-direct-only": "仅直连",
		"voice-error-mic-permission": "无法使用麦克风",
		"voice-error-disabled": "本服务器未启用语音",
		"voice-error-room-full": "语音人数已满",
		"voice-error-too-many-rooms": "同时语音的房间已达上限",
		"voice-error-budget": "本月语音中继额度已用尽",
		tabs: {
			queue: "队列",
			notes: "便签",
			settings: "设置",
		},
		users: {
			rename: "改名",
			"rename-hint": "修改你的用户名或访客昵称",
			title: "用户",
			set: "设置你的名字",
			empty: "这里似乎没有其他人。邀请一些朋友吧！",
			you: "你",
			demote: "降级",
			promote: "提升",
			kick: "踢出",
			"role-aria": "{name} 的身份：{detail}",
			"player-status-aria": "{name} 的播放状态：{detail}",
			"player-status": {
				none: "暂无状态",
				ready: "已就绪",
				buffering: "缓冲中",
				error: "播放出错",
			},
		},
	},
	privacy: {
		title: "@:footer.privacy-policy",
	},
	help: {
		title: "帮助与常见问题",
		intro: "这里是常用功能的简要说明。房间内还会就地提示播放手势、快捷键和错误原因。",
		add: {
			title: "我能添加什么？",
			text: "添加时会先读取片源的标题、时长等信息，随后由每位观众的浏览器直接加载片源。链接必须公开可访问，不能是本地文件。",
			items: {
				youtube: "YouTube 视频、播放列表和频道（搜索功能可用时）",
				vimeo: "Vimeo 视频",
				direct: "MP4、WebM 等视频直链",
				hls: "HLS（.m3u8）直播或点播列表",
				dash: "DASH（.mpd）清单",
				manifest: "自定义媒体清单（.json）",
				bilibili:
					"B 站视频（支持 BV/av 号、分 P 与 b23.tv 短链；外链播放器无法被程序控制，切换视频与进入房间时会对齐，播放中各观众自主控制，进度不做强制同步）",
			},
			note: "需要登录或带 DRM 的付费站点、以及只放行站内来源的链接通常无法播放。",
		},
		playback: {
			title: "播放与同步",
			text: "播放、暂停和跳转对整个房间生效。新观众打开链接时，浏览器可能拦截自动播放，此时点击画面上的“点击加入播放”即可跟上房间进度。",
		},
		buffering: {
			title: "一直缓冲或播放失败怎么办？",
			items: {
				cors: "片源未允许跨域访问（CORS）时，本站会自动关闭画质增强并尝试直接播放；仍然失败请更换片源。",
				unreachable:
					"提示“连不上视频源”时，是你的浏览器到不了源站（被网络屏蔽、DNS 异常或代理分流走错线路），与服务器无关：服务器能探测到它，不代表你的设备也能访问。",
				retry: "错误提示中的“仅重载我的视频”只重载你本机，不影响房间里的其他人。",
				gate: "房间设置中的“等待观众缓冲”可以让所有人等一下缓冲慢的观众。",
				source: "浏览器直接打开链接能播放，不代表可以嵌入其他站点播放。只拦站外 Referer 的源会自动改用不带 Referer 的请求；必须来自站内来源或需要 Cookie 的链接仍然不可用。",
			},
		},
		enhance: {
			title: "画质增强",
			text: "“清晰化”使用 GPU 做边缘自适应放大与锐化，自动倍率不会低于原分辨率；“AI 超分（Anime4K）”需要 WebGPU。设备性能不足时会自动降档；片源未允许跨域或不支持 WebGPU 时无法启用。",
		},
		shortcuts: {
			title: "快捷键与手势",
			text: "空格 / K 播放或暂停，← / → 与 J / L 按设置的时长跳转，长按 → 进入 2 倍速，F 全屏，M 静音，T 打开聊天，按 ? 查看完整说明。手机双击画面暂停或继续，左右滑动跳转，长按约 0.5 秒进入 2 倍速。",
		},
		rooms: {
			title: "房间与身份",
			text: "临时房间在无人后自动回收；永久房间会保存视频、队列和进度。游客昵称由系统随机生成，可在用户列表中修改。",
		},
		contact: {
			title: "还有问题？",
			text: "可以先查阅上游的英文文档与讨论，或在问题追踪中报告 Bug（需要 GitHub 账号）。",
			upstream: "上游文档与讨论（英文）",
			bug: "报告 Bug",
		},
	},
	chat: {
		title: "聊天",
		open: "展开聊天（不会自动打开键盘）",
		close: "收起聊天",
		"type-here": "在此输入你的消息…",
		send: "发送消息",
		"to-bottom": "跳到最新消息",
		empty: "还没有消息，打个招呼开始吧！",
		"empty-hint": "新消息提示音默认关闭，可在偏好设置中开启。",
		emoji: {
			open: "插入表情",
			quick: "常用表情",
			groups: {
				smileys: "笑脸",
				gestures: "手势",
				animals: "动物",
				food: "食物",
				activities: "活动",
				symbols: "符号",
			},
		},
	},
	"room-notes": {
		"input-label": "添加便签",
		"input-hint": "便签房间内所有人可见；只能追加和删除，不能修改。",
		placeholder: "写点什么，比如今晚看什么、下次聚会的时间…",
		add: "添加便签",
		empty: "还没有便签。",
		counter: "{count} / {max} 条",
		full: "便签数量已达上限，删除一些后再添加。",
		"delete-label": "删除便签",
	},
	"room-event": {
		joined: "{user} 加入了房间",
		left: "{user} 离开了房间",
		played: "{user} 播放了视频",
		paused: "{user} 暂停了视频",
		skipped: "{user} 切到了 {video}",
		seeked: "{user} 跳转到 {time}",
		added: "{user} 添加了 {video}",
		"added-many": "{user} 添加了 {count} 个视频",
		"added-unknown": "{user} 添加了一个视频",
		removed: "{user} 移除了 {video}",
		"removed-unknown": "{user} 移除了一个视频",
		updated: "{user} 更新了 {video} 的设置",
		"updated-unknown": "{user} 更新了某个视频的设置",
		unknown: "房间发生了一个操作",
	},
	"share-invite": {
		title: "分享邀请",
		text: "复制此链接并分享给你的朋友！",
		copied: "已复制！",
	},
	video: {
		"add-explanation": "添加到队列。",
		"full-link": "完整链接：{url}",
		playnow: "立即播放",
		"playnow-explanation": "立即播放此视频，并将当前视频置于队列顶部。",
		"no-video": "没有正在播放的视频。",
		"no-video-text": "点击下方的“添加”来添加视频。",
	},
	"add-preview": {
		label: "链接或搜索",
		canceled: "已取消读取视频信息。",
		loading: "正在读取视频信息…",
		"loading-slow": "片源响应较慢，仍在读取视频信息…",
		retry: "重试",
		"add-all": "全部添加",
		placeholder: "输入以搜索 YouTube，或输入视频网址添加到队列",
		"placeholder-no-search": "输入视频网址添加到队列",
		title: "我可以添加什么？",
		"single-videos": "单个视频",
		playlists: "播放列表",
		playlist: "播放列表",
		text: "或直接输入文字来搜索 Youtube。",
		"text-no-search": "搜索功能未启用，请直接粘贴视频网址。",
		"search-for": "按回车或点击搜索，在 YouTube 上搜索“{search}”。",
		platforms: {
			"youtube-videos": "Youtube 视频：{url}",
			"vimeo-videos": "Vimeo 视频：{url}",
			"any-mp4-videos": "任何公开的 .mp4 视频：{url}",
			"youtube-playlists": "Youtube 播放列表：{url}",
			"youtube-channels": "Youtube 频道：{url}",
		},
		messages: {
			timeout: "读取视频信息超时（45 秒）。链接可能仍可播放，请重试。",
			"metadata-timeout": "片源的视频信息读取超时。链接可能仍可播放，请重试。",
			"unknown-status": "添加预览响应的未知状态：{status}。",
			"unknown-error": "获取添加预览时发生未知错误。请稍后重试。",
			"failed-to-get-add-preview": "获取添加预览失败。这可能是个 Bug，请查看控制台了解详情。",
			"failed-to-all-videos": "添加所有视频失败：{message}",
		},
		"adapter-selector": {
			label: "强制使用适配器",
			auto: "自动检测",
		},
		"same-series": "同剧集：还找到 {count} 集",
		"same-series-add": "加入队列",
		"render-target": "渲染后画质：{size}（{scale}×）",
	},
	"processed-text": {
		"link-hint": "点击复制此链接到添加标签页。",
	},
	"video-queue": {
		"no-videos": "还没有待播视频。正在播放的视频显示在播放器上方。",
		"add-video": "添加视频",
		shuffle: "随机排序队列",
		"item-actions": "视频操作菜单",
		export: "导出",
		"export-diag-title": "导出队列",
		"export-hint": "复制并粘贴此文本到“添加”标签页，即可恢复此队列。",
		restore: "你想恢复上一个队列中的视频吗？",
		"restore-queue": "恢复队列？",
		"restore-queue-hint": "这是上次该房间活跃时队列中的内容。你想恢复它吗？",
	},
	"video-queue-item": {
		experimental: "此服务为实验性支持！预计会经常出问题。",
		"media-access-warning": "源站只放行来自站内的请求，这里可能无法播放。",
		"play-next": "下一个播放",
		"play-last": "最后播放",
		messages: {
			"video-added": "视频已添加",
			"video-removed": "视频已移除",
			"video-updated": "视频已更新",
		},
		"start-at": "从 {timestamp} 开始",
		edit: {
			title: "编辑视频",
			"subtitle-url": "字幕网址 (.vtt)",
			"subtitle-url-supported-services": "支持的服务：direct、googledrive",
			tooltip: "编辑视频设置",
		},
	},
	"room-settings": {
		title: "@:create-room-form.title",
		description: "@:create-room-form.description",
		visibility: "@:create-room-form.visibility",
		public: "@:create-room-form.public",
		unlisted: "@:create-room-form.unlisted",
		private: "私密",
		"queue-mode": "@:create-room-form.queue-mode",
		manual: "@:create-room-form.manual",
		"manual-hint": "默认的常规行为，符合你的预期。你可以手动重新排列队列中的项目。",
		vote: "@:common.vote",
		"vote-hint": "得票最高的视频将被下一个播放。",
		loop: "循环",
		"loop-hint": "视频结束时，把它放到队列末尾。",
		dj: "DJ",
		"dj-hint": "视频结束时，从头开始播放同一视频。适合循环播放背景音乐。",
		"auto-skip-text": "使用 SponsorBlock 数据自动跳过不需要的视频片段。",
		"auto-skip-text-sponsor": "赞助",
		"auto-skip-text-intro": "片头",
		"auto-skip-text-outro": "片尾",
		"auto-skip-text-interaction": "互动提醒",
		"auto-skip-text-selfpromo": "自我推广",
		"auto-skip-text-music_offtopic": "无关音乐",
		"auto-skip-text-preview": "预览",
		"permissions-not-available": "临时房间中无法使用权限设置。",
		"room-needs-owner": "此房间需要先有一个房主，才能修改权限。",
		"login-to-claim": "登录以认领此房间。",
		claim: "认领房间",
		"arent-able-to-modify-permissions": "你无法修改此房间的权限。",
		"settings-applied": "设置已应用",
		"now-own-the-room": "你现在拥有房间 {room}。",
		"load-failed": "加载房间设置失败。",
		"restore-queue": "房间加载时恢复队列",
		"enable-vote-skip": "启用投票跳过",
		"buffer-gate": "等待观众缓冲",
		"buffer-gate-off": "关闭",
		"buffer-gate-pause": "自动暂停，最多等待 15 秒",
		"buffer-gate-hint":
			"有播放权限的观众缓冲时一起暂停；后台标签页不参与等待。超时后继续播放，30 秒内不再自动暂停。",
	},
	"create-room-form": {
		"card-title": "创建永久房间",
		"create-room": "创建房间",
		"create-failed": "创建房间失败，请稍后重试。",
		name: "名称",
		"name-hint": "用于房间网址。之后无法更改。",
		title: "标题",
		"title-hint": "可选",
		description: "描述",
		"description-hint": "@:create-room-form.title-hint",
		visibility: "可见性",
		"visibility-hint": "控制房间是否显示在房间列表中。",
		"queue-mode": "队列模式",
		manual: "手动",
		vote: "@:common.vote",
		loop: "@:room-settings.loop",
		dj: "@:room-settings.dj",
		public: "公开",
		unlisted: "不公开",
		rules: {
			name: {
				"name-required": "名称为必填项",
				"no-spaces": "名称不能包含空格。",
				length: "名称长度必须在 3 到 32 个字符之间",
				alphanumeric: "名称只能包含字母数字字符、连字符和下划线",
				taken: "名称已被占用",
			},
			"invalid-visibility": "无效的可见性",
			"invalid-queue": "无效的队列模式",
		},
		"unknown-error": "发生未知错误。请稍后重试。",
	},
	"login-form": {
		login: "@:nav.login",
		register: "注册",
		"login-discord": "使用 Discord 登录",
		email: "邮箱",
		"email-or-username": "邮箱或用户名",
		username: "用户名",
		password: "密码",
		"retype-password": "再次输入密码",
		"email-optional": "可选。提供邮箱后，如果你忘记密码，可以用它恢复账户。",
		rules: {
			"email-required": "邮箱为必填项",
			"valid-email": "必须是有效的邮箱",
			"username-required": "用户名为必填项",
			"username-length": "用户名长度必须在 1 到 {length} 个字符之间",
			"password-required": "密码为必填项",
			"password-length": "密码长度至少为 10 个字符",
			"retype-password": "请再次输入你的密码",
			"passwords-match": "两次输入的密码必须一致",
		},
		errors: {
			"something-weird-happened": "发生了奇怪的事情，但你可能已经登录了？刷新页面看看。",
			"invalid-credentials": "邮箱或密码不正确。",
			"login-failed-noserver": "登录失败，但服务器没有说明原因。请将此报告为 Bug。",
			"login-failed": "登录失败，我也不知道为什么。请将此报告为 Bug。",
			"register-failed-noserver": "注册失败，但服务器没有说明原因。请将此报告为 Bug。",
			"register-failed": "注册失败，我也不知道为什么。请检查控制台并将此报告为 Bug。",
			"in-use": "已被使用。",
		},
		"change-password": {
			title: "修改密码",
			success: "密码修改成功。",
			forgot: "忘记密码？",
			prompt: "输入与你账户关联的邮箱地址或用户名。",
			reset: "重置",
			sent: "密码重置邮件已发送。",
			failed: "无法重置密码。",
		},
	},
	"permissions-editor": {
		title: "权限编辑器",
		text1: "授予权限较低用户的所有权限，都会自动授予权限较高的用户。",
		text2: "管理员拥有一切权限。房主自动成为管理员，且不能被降级。",
		"viewing-as": "查看身份",
		permission: "权限",
		"group-count": "{count} 项",
		groups: {
			playback: "播放",
			queue: "队列",
			chat: "聊天",
			room: "房间设置",
			users: "用户管理",
			advanced: "高级：权限委派",
		},
	},
	permissions: {
		playback: {
			"play-pause": "播放 / 暂停",
			skip: "跳过当前视频",
			seek: "调整播放进度",
			speed: "调整播放速度",
		},
		chat: "发送聊天消息",
		"manage-queue": {
			add: "添加视频",
			remove: "移除视频",
			order: "调整队列顺序",
			vote: "参与投票",
			"play-now": "立即播放视频",
			edit: "编辑队列中的视频（字幕等）",
		},
		"configure-room": {
			"set-title": "修改房间标题",
			"set-description": "修改房间描述",
			"set-visibility": "修改房间可见性",
			"set-queue-mode": "修改队列模式",
			other: "修改其他房间设置（自动跳过、缓冲等待等）",
			"set-permissions": {
				"for-moderator": "设置协管员的权限",
				"for-trusted-users": "设置受信任用户的权限",
				"for-all-registered-users": "设置注册用户的权限",
				"for-all-unregistered-users": "设置未注册用户的权限",
			},
		},
		"manage-users": {
			"promote-admin": "提升为管理员",
			"demote-admin": "撤销管理员",
			"promote-moderator": "提升为协管员",
			"demote-moderator": "撤销协管员",
			"promote-trusted-user": "提升为受信任用户",
			"demote-trusted-user": "撤销受信任用户",
			kick: "踢出用户",
		},
	},
	"client-settings": {
		title: "偏好设置",
		description: "这些设置保存在你的浏览器中，只影响你自己。",
		activator: "@:client-settings.title",
		"room-layout": "房间布局",
		layouts: {
			default: "@:room.default-layout",
			theater: "@:room.theater-mode",
		},
		theme: "主题",
		themes: {
			dark: "深色",
			light: "浅色",
			deepred: "暗红",
			deepblue: "深蓝",
			greenslate: "灰绿",
			strawberry: "草莓红",
		},
		"playback-preferences": "观看偏好",
		"chat-overlay-duration": "消息停留时长",
		"chat-overlay-hint": "新消息在画面上的显示时间；关闭后仍可展开聊天查看历史。",
		"presence-notice-duration": "用户进出提醒时长",
		"seek-notice-duration": "跳进度提醒时长",
		"room-notice-hint": "可缩短显示时间或关闭这类提醒，只影响你自己。",
		"controls-hide-delay": "控件自动隐藏时间",
		"controls-hide-hint": "无操作多久后收起控件；电脑全屏时同时隐藏鼠标指针。",
		"hls-buffer-duration": "预缓冲时长",
		"hls-buffer-hint":
			"适用于 HLS（.m3u8）与 DASH（.mpd）：这是目标值，网络允许时会展缓存更多（直到设备内存的字节上限）；MP4 与原生 HLS 的缓存由浏览器管理。缓存越长占用内存和流量越多。",
		"audio-boost": "音频增强",
		"audio-boost-hint": "将支持的非 iframe 播放器的音频增强至 300%。音效不受影响。",
		"audio-boost-unsupported": "当前播放器不支持音频增强。音效不受影响。",
		"sfx-enable": "新消息提示音",
		"sfx-hint": "默认关闭，只影响新消息提示音，不改变视频音量。",
		"sfx-volume": "提示音音量",
		"room-settings": "默认房间设置",
		"enable-adapter-selector": "显示适配器选择器（高级）",
	},
	"client-update": {
		text: "已发布新版本。",
		action: "刷新页面",
		dismiss: "稍后再说",
	},
	"connect-overlay": {
		title: "已断开连接",
		"find-another": "寻找其他房间",
		reconnect: "重新连接",
		"dc-reasons": {
			[OttWebsocketError.UNKNOWN]: "@:connect-overlay.dc-reasons.unknown",
			[OttWebsocketError.ROOM_NOT_FOUND]: "房间不存在或已关闭。",
			[OttWebsocketError.ROOM_UNLOADED]: "房间因长时间无人而关闭。",
			[OttWebsocketError.MISSING_TOKEN]: "登录状态已失效。请刷新页面后重试。",
			[OttWebsocketError.KICKED]: "你被房主或管理员踢出了房间。",
			unknown: "连接被中断，可能是网络波动或服务端重启。可以尝试重新连接，或刷新页面。",
		},
	},
	"vote-skip": {
		remaining: "还需 {count} 票即可跳过",
	},
	roles: {
		[Role.Administrator]: "管理员",
		[Role.Moderator]: "协管员",
		[Role.TrustedUser]: "受信任用户",
		[Role.RegisteredUser]: "注册用户",
		[Role.UnregisteredUser]: "未注册用户",
		[Role.Owner]: "房主",
	},
	errors: {
		unknown: "操作失败，请稍后再试。",
		network: "网络请求失败，请检查网络后重试。",
		"session-expired": "登录状态已失效，请重新登录后再试。",
		AlreadyInUse: "该项已被使用。",
		ValidationError: "提交的信息不符合要求，请检查后重试。",
		InvalidPassword: "当前密码不正确。",
		CurrentPasswordRequired: "需要输入当前密码。",
		BadPasswordError:
			"密码不符合最低要求。长度必须至少为 8 个字符，且包含以下字符类别中的至少 2 种：小写字母、大写字母、数字、特殊字符。",
		BadApiArgumentException: "API 参数错误。这很可能是个 Bug，请报告。",
		FeatureDisabledException: "该功能当前不可用。",
		LocalFileException: "无法播放本地文件，请使用所有人可访问的公开链接。",
		MissingMetadataException: "无法获取视频时长等必要信息，请更换一个片源。",
		OutOfQuotaException: "服务额度不足，请稍后再试。",
		UnsupportedServiceException: "暂不支持此链接，请确认它指向受支持的视频服务或视频直链。",
		UnsupportedMimeTypeException: "此文件的格式不受支持。",
		UnsupportedVideoType: "暂不支持此类型的视频。",
		UnsupportedSubtitleType: "字幕链接必须以 .vtt 结尾。",
		InvalidVideoIdException: "视频链接无效，请检查后重试。",
		InvalidAddPreviewInputException: "请多输入几个字符，或直接粘贴视频链接。",
		FfprobeError: "无法读取视频信息，请确认链接可公开访问后重试。",
		FfprobeTimeoutError: "读取视频信息超时，链接可能仍可播放，请重试。",
		TooManyNotesException: "便签数量已达上限，请先删除一条再添加。",
		NoteTooLongException: "便签内容过长，请精简后再发送。",
		NoteNotFoundException: "这条便签已不存在，可能已被其他人删除。",
		M3u8ParseError: "无法解析 HLS 播放列表，请检查链接。",
		PermissionDeniedException: "你没有执行此操作的权限。",
		VideoAlreadyQueuedException: "该视频已在队列中。",
		VideoNotFoundException: "该视频不存在或已被移除。",
		RoomNotFoundException: "房间不存在或已关闭。",
		RoomAlreadyLoadedException: "房间已加载，请刷新页面后重试。",
		RoomNameTakenException: "该房间名称已被占用，请换一个。",
		ClientNotFoundInRoomException: "服务器找不到你的连接，请刷新页面后重试。",
		MissingToken: "登录状态已失效，请刷新页面后重试。",
		InvalidTokenException: "登录状态已失效，请刷新页面后重试。",
	},
	player: {
		"now-playing": "正在播放",
		settings: {
			disabled: "不可用",
			unknown: "未知",
			auto: "自动",
			"auto-with-value": "自动（{value}）",
		},
		"join-playback": {
			title: "房间正在播放",
			action: "点击加入播放",
			hint: "浏览器需要你先点击一次，才能开始播放。",
		},
		episode: "第 {episode} 集",
		"season-episode": "第 {season} 季 · 第 {episode} 集",
		interactions: {
			"toggle-controls": "显示或收起播放控件",
			"seek-step": "跳转时长（滑动 / 方向键）",
			seconds: "{count} 秒",
			"seek-preview": "{delta} 秒 → {time}，松手跳转",
			"seek-denied": "你没有调整房间播放进度的权限",
			"play-pause-denied": "你没有播放或暂停房间视频的权限",
			"speed-denied": "你没有调整房间倍速的权限",
			"speed-busy": "其他观众正在长按倍速播放",
			"speed-unavailable": "当前无法使用长按倍速",
			holding: "2 倍速播放中，松手恢复",
			"room-holding": "房间正在临时 2 倍速播放",
		},
		shortcuts: {
			title: "播放手势与快捷键",
			gestures:
				"点击画面显示或收起控件；手机双击画面暂停或继续播放。左右滑动按设定时长跳转，松手执行；长按约 0.5 秒进入 2 倍速，松手恢复。电脑双击画面切换全屏。",
			"chat-hint":
				"电脑按 Enter 展开聊天并开始输入，再按 Enter 发送。点击聊天按钮或按 T 只展开阅读，不会自动弹出键盘；按 Escape 收起聊天并保留未发送的草稿。",
			"play-pause": "播放 / 暂停",
			"seek-step": "按设置的时长后退 / 前进",
			"speed-hold": "长按进入 2 倍速，松手恢复",
			volume: "音量增加 / 减少 5%",
			mute: "静音 / 取消静音",
			fullscreen: "切换播放器全屏",
			escape: "关闭当前浮层 / 退出全屏",
			chat: "展开 / 收起聊天",
			"chat-type": "打开聊天输入 / 发送消息",
			start: "跳到视频开头",
			skip: "跳过当前视频",
			help: "显示这份说明",
			stats: "查看视频详情",
			scope: "播放、跳转和倍速同步全房间，并遵守房间权限。音量、聊天和全屏只影响你的设备。输入文字、使用输入法或操作弹窗时，不会触发播放快捷键。直播等不可拖动的视频不支持滑动跳转。",
		},
		stats: {
			title: "视频详情",
			hint: "在画面任意位置点右键，也可以直接打开这个面板。",
			"section-video": "视频",
			"section-playback": "播放",
			"section-enhancement": "画质增强",
			"section-device": "设备与环境",
			resolution: "分辨率",
			time: "进度 / 时长",
			"value-time": "{current} / {total}",
			"source-type": "片源类型",
			"source-host": "源主机",
			state: "状态",
			"state-playing": "播放中",
			"state-paused": "已暂停",
			"state-buffering": "缓冲中",
			"state-unknown": "嵌入播放器",
			rate: "播放倍速",
			"buffer-ahead": "已缓冲前瞻",
			"value-seconds": "{value} 秒",
			dropped: "丢帧",
			"render-fps": "实测帧率",
			drift: "与房间偏差",
			mode: "增强模式",
			"render-target": "渲染目标",
			"enhancement-target": "增强渲染中",
			"enhancement-error": "增强错误",
			"auto-degrade": "自动降档",
			"max-scale": "增强倍率上限",
			viewport: "视口 / DPR",
			webgpu: "WebGPU",
			available: "可用",
			unavailable: "不可用",
		},
		"buffer-warn": {
			spans: "你还没有缓冲足够的视频内容。当前已缓冲的时间范围：{ranges}",
		},
		"playback-error-title": {
			unknown: "播放错误",
			network: "视频网络连接异常",
			decode: "视频解码失败",
			unsupported: "无法播放此视频源",
		},
		"playback-error-message": {
			unknown: "加载视频失败。",
			network: "视频请求失败。请检查网络、代理分流及视频源是否可访问后重试。",
			decode: "浏览器无法解码当前视频。可重试加载，或尝试其他浏览器或视频源。",
			unsupported:
				"可能是链接已失效、视频格式不受支持，或源站限制了防盗链/跨域访问。可点击“仅重载我的视频”重试，或请添加者更换片源。",
			"unsupported-unreachable":
				"你这台设备连不上视频源。服务器能探测到它并不代表你的浏览器也能访问：常见原因是源站被网络屏蔽，或代理规则把它分流到了走不通的线路。请检查网络与代理分流后重试。",
		},
		"cors-fallback": "已暂时关闭画质增强以保证播放（该片源可能不允许跨域访问）。",
		recovery: {
			skipped: "此处片源取不到数据，已跳过 {seconds} 秒继续播放。",
		},
		"retry-local": "仅重载我的视频",
		"retry-local-hint": "仅重新加载你这台设备上的视频，不改变房间的播放状态。",
		"notes-collapse": "收起便签",
		"notes-expand": "展开便签",
		loading: {
			preparing: "正在准备视频",
			seeking: "正在加载房间当前进度",
			buffering: "正在缓冲视频",
			"waiting-frame": "正在等待画面",
			"waiting-viewer": "等待首位观众准备好",
			"wait-together": "大家一起开始：首位观众的画面准备好后房间才开始计时，不会漏掉开头。",
			"resume-saved": "正在准备画面：等到首帧就绪后房间才开始计时，大家从同一位置同时开始。",
			"resume-retry": "恢复播放需要重试",
			"resume-failed": "暂时无法恢复保存的进度，请重载本机视频后重试。",
			elapsed: "已等待 {seconds} 秒",
			"buffer-ahead": "本机当前位置已缓存 {seconds} 秒",
			"join-playing": "房间正在播放，画面准备好后会自动跟上。",
			"join-paused": "视频画面准备好后会自动显示。",
			slow: "加载时间较长。片源和网络速度会影响等待时间，可以继续等待或重试。",
		},
	},
};
