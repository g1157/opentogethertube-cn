...
此文档适用于 `v0.15.0-cn7`。每次发布都先在独立的 18080 端口验收，通过后将同一个已验收镜像
...
`ott-next:0.15.0-cn7`；下面的独立验收示例显式使用 `ott-preview:0.15.0-cn7`，避免覆盖现有镜像标签。
...
OTT_IMAGE=ott-preview:0.15.0-cn7
